	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (Varyings i) : SV_Target
			{
				...
				float shadowAttenuation = GetShadowAttenuation(i, normal, worldLightPos, viewDir);
				...
				specularAdjustment = drawnSpecular * specularAdjustment.xyz + 1;
				specularAdjustment = diffuse.rgb * specularAdjustment.xyz + drawnSpecularColor;

				float specularPowerMesh;
				float specularMesh = GetMeshSpecular(normal, viewDir, worldLightPos, specularPowerMesh);
				float3 specularColorMesh = _SpecularColor.xyz * specularPowerMesh + diffuse;

				diffuse = diffuse * _LineColorG.xyz;
				specularColorMesh =  -specularAdjustment.xyz + specularColorMesh;
				float3 specularDiffuse = saturate(_notusetexspecular * specularColorMesh + specularAdjustment.xyz);				

				//Shading 
				//I'm too lazy to organize this part shit is all over the place
				...
				float3 cumCol = (GetLambert(worldLightPos, normal) + 0.5) * float3(0.149999976, 0.199999988, 0.300000012) + float3(.850000024, 0.800000012, 0.699999988);
				...
				bodyShine = _LightColor0.rgb * float3(0.600000024, 0.600000024, 0.600000024) + float3(0.400000006, 0.400000006, 0.400000006);
