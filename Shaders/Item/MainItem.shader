	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (Varyings i, int faceDir : VFACE) : SV_Target
			{
				...
				float2 anotherRampUV = specular * _AnotherRamp_ST.xy + _AnotherRamp_ST.zw;
				...
				specular = specular * detailMask.x - drawnSpecular;
				specular = _notusetexspecular * specular + drawnSpecular;
				diffuseShadow = diffuseShadow + specular;
